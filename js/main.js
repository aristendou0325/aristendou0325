...
/**
 * 从localStorage加载文章并添加到页面
 * 功能：读取用户上传到localStorage的文章数据，动态生成HTML并添加到文章列表
 * @returns {void}
 */
function loadLocalArticles() {
    ...
    // 获取文章列表容器
    let postsContainer = contentWrapper.querySelector('.local-posts-container');
    // 如果不存在，则创建
    if (!postsContainer) {
        // 先保存分页元素（如果存在）
        const pagination = contentWrapper.querySelector('.pagination');
        
        // 创建文章容器
        postsContainer = document.createElement('div');
        postsContainer.className = 'local-posts-container';
        
        // 在分页前插入容器
        if (pagination) {
            contentWrapper.insertBefore(postsContainer, pagination);
        } else {
    ...
    // 从localStorage获取文章
    try {
        const articlesData = localStorage.getItem('blogArticles');
        if (articlesData) {
            ...
            if (articles && articles.length > 0) {
                ...
                postsContainer.innerHTML = '<p class="no-posts">暂无本地文章</p>';
            }
        }
    } catch (error) {
        console.error('加载本地文章失败:', error);
        postsContainer.innerHTML = '<p class="error-message">加载文章时出错</p>';
    }
}
...
function createPostElement(article) {
    ...
    // 生成文章HTML内容，使用URL参数形式的链接
    const articleLink = article.id ? `article.html?id=${article.id}` : '#';
    const articleHtml = `
        <div class="post-header">
            <h2 class="post-title"><a href="${articleLink}">${article.title}</a></h2>
            <div class="post-meta">
                <span class="post-date">${article.date}</span>
                <span class="post-author">${article.author || 'airs'}</span>
                <span class="post-category"><a href="#">${article.category}</a></span>
                <span class="post-comments">${article.comments || 0} 条评论</span>
            </div>
        </div>
        <div class="post-content">
            <p>${article.summary || truncateText(article.content, 150)}</p>
        </div>
        <div class="post-footer">
            <a href="${articleLink}" class="read-more">阅读更多</a>
            <div class="post-tags">
                ${article.tags ? article.tags.map(tag => `<a href="#">${tag}</a>`).join('') : ''}
            </div>
        </div>
    `;
